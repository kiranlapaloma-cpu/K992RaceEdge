...
# ======================= Pace Elasticity (PE) — RAM-light table =======================
st.markdown("## Ability × Adaptability — GCI with Pace Elasticity")

import numpy as np
import pandas as pd

# ---- small helpers (local; no globals polluted) -------------------------------------
def _num(s):
    return pd.to_numeric(s, errors="coerce")

def _time_cols(df):
    # pick "*_Time" but not variants like "RaceTime_s"
    cols = [c for c in df.columns if isinstance(c, str) and c.endswith("_Time") and c[:-5].isdigit()]
    # sort by numeric marker ascending (e.g., 100,200,...), which corresponds to later segments to earlier from-home
    return sorted(cols, key=lambda c: int(c.split("_")[0]))

def _segment_spec(work_df, D, step):
    """
    Build segments [(label, length_m, src_col)] in race order (left→right plot order):
    D→m1 uses m1_Time; then m1→m2 uses m2_Time; ... ; finish uses Finish_Time.
    """
    cols = _time_cols(work_df)
    if not cols and "Finish_Time" not in work_df.columns:
        return []

    # numeric markers present (e.g., ['1200_Time','1100_Time',...]) → [1200,1100,...]
    marks = [int(c.split("_")[0]) for c in cols]
    marks = sorted(set(marks), reverse=False)  # 100,200,300,... (from late to early when read)
    # Turn into from-start chronology: earliest segment first.
    # Our convention (like your pace curve): list in chronological order from early → late.
    # The markers are metres-from-home; chronological order is decreasing from D down to 0.
    marks_desc = sorted(marks, reverse=True)  # e.g., 1500,1400,...,100

    segs = []
    if marks_desc:
        m1 = marks_desc[0]
        L0 = max(1.0, float(D) - float(m1))
        if f"{m1}_Time" in work_df.columns and L0 > 0.5:
            segs.append((f"{int(D)}→{m1}", L0, f"{m1}_Time"))
        for a, b in zip(marks_desc, marks_desc[1:]):
            src = f"{int(b)}_Time"
            if src in work_df.columns and (a - b) > 0.5:
                segs.append((f"{int(a)}→{int(b)}", float(a - b), src))

    if "Finish_Time" in work_df.columns:
        fin_len = 100.0 if int(step) == 100 else 200.0
        segs.append((f"{int(step)}→0 (Finish)", fin_len, "Finish_Time"))

    return segs

def _elasticity_scores(field_d, horse_d):
    """Return (corr, slope_tanh) given field deltas and horse deltas (1D arrays with NaNs handled)."""
    f = np.asarray(field_d, dtype=float)
    h = np.asarray(horse_d, dtype=float)
    ...
    fz = (f - f.mean())
    hz = (h - h.mean())
    denom = (np.sqrt((fz**2).sum()) * np.sqrt((hz**2).sum()))
    corr = float((fz * hz).sum() / denom) if denom > 0 else np.nan
    corr = np.clip(corr, -1.0, 1.0)

    # Slope of h ~ a + b*f  (OLS, robust to scale). Then squash via tanh to keep bounded.
    var_f = float((fz**2).sum())
    slope = float((fz * hz).sum() / var_f) if var_f > 0 else np.nan
    slope_tanh = np.tanh(slope) if np.isfinite(slope) else np.nan
    return corr, slope_tanh

# ---- inputs & safeguards ------------------------------------------------------------
STEP = int(metrics.attrs.get("STEP", 100))
D    = float(st.session_state.get("race_distance_m", metrics.attrs.get("RACE_DISTANCE", 1200) or 1200))
IGNORE_FIRST_SPLIT = True  # first split is often noisy; you asked for this behaviour
MIN_SEGMENTS       = 4     # need at least 4 segments to have 3 deltas

# We need the raw timing table that has the *_Time columns.
# If your app keeps it as `work` (like your pace curve block), we’ll use it; else fall back to `metrics`.
raw_df = 'work' in globals() and isinstance(work, pd.DataFrame) and (len([c for c in work.columns if c.endswith("_Time")]) > 0)
timing = work if raw_df else metrics

# Build segment spec
segs = _segment_spec(timing, D, STEP)
if len(segs) < MIN_SEGMENTS:
    st.info("Pace Elasticity: not enough timing segments to estimate elasticity reliably.")
else:
    # Build per-segment speed matrix (horses × segments) without materialising big copies
    seg_keys = [f"seg_{i}" for i in range(len(segs))]
    spd = pd.DataFrame(index=timing.index, columns=seg_keys, dtype="float32")

    for key, (_, L, src) in zip(seg_keys, segs):
        t = _num(timing.get(src))
        t = t.mask((t <= 0) | (~np.isfinite(t)))
        spd[key] = (float(L) / t).astype("float32")  # m/s per segment

    # Field average per segment
    field_avg = spd.mean(axis=0, skipna=True).astype("float32").to_numpy()

    # Build delta series (segment-to-segment change). Optionally drop first delta.
    def deltas(vec):
        v = np.asarray(vec, dtype=float)
        d = np.diff(v)
        return d[1:] if IGNORE_FIRST_SPLIT and d.size >= 2 else d  # drop first delta if requested

    field_d = deltas(field_avg)

    # Per-horse elasticity vs field deltas
    corr_list, slope_list, pe_list = [], [], []

    for i in range(len(spd)):
        row = spd.iloc[i].to_numpy(dtype=float)
        h_d = deltas(row)

        c, s = _elasticity_scores(field_d, h_d)
        # Combine: 0.6*correlation + 0.4*slope_tanh, scaled to 0..100
        if np.isfinite(c) and np.isfinite(s):
            pe01 = 0.6 * ((c + 1.0) / 2.0) + 0.4 * ((s + 1.0) / 2.0)
            pe = float(np.clip(100.0 * pe01, 0.0, 100.0))
        else:
            pe = np.nan
        corr_list.append(c)
        slope_list.append(s)
    ...
    # Small stability penalty for extremely jumpy horses (doesn’t add heavy RAM)
    # Use per-horse delta variance vs field delta variance
    fdv = np.nanvar(field_d) if np.isfinite(field_d).any() else 0.0
    adj = []
    for i in range(len(spd)):
        row = spd.iloc[i].to_numpy(dtype=float)
        h_d = deltas(row)
        hdv = np.nanvar(h_d) if np.isfinite(h_d).any() else np.nan
        if not np.isfinite(hdv) or fdv <= 0:
            adj.append(1.0)
        else:
            # if horse is much noisier than field, trim by up to 10%
            ratio = float(hdv / fdv)
            trim = np.clip((ratio - 1.5) / 6.0, 0.0, 0.10)  # gentle
            adj.append(1.0 - trim)

    pe_adj = [None if not np.isfinite(v) else float(np.clip(v * a, 0.0, 100.0)) for v, a in zip(pe_list, adj)]

    # Assemble view (pull GCI if present; else blank)
    G = metrics.get("GCI_RS", metrics.get("GCI"))
    gci = _num(G) if G is not None else pd.Series([np.nan]*len(metrics), index=metrics.index, dtype="float32")

    view = pd.DataFrame({
        "Horse": metrics.get("Horse", pd.Series(np.arange(len(metrics)), dtype=object)),
        "GCI": gci.round(2),
        "PE": pd.Series(pe_adj, index=metrics.index, dtype="float32").round(1),
        "PE_corr": pd.Series(corr_list, index=metrics.index, dtype="float32").round(2),
        "PE_slope": pd.Series(slope_list, index=metrics.index, dtype="float32").round(2),
    })

    # Class label
    def _pe_label(x):
        if not np.isfinite(x): return ""
        if x >= 75: return "High elasticity"
        if x >= 55: return "Moderate elasticity"
        return "Low elasticity"

    view["PE_class"] = view["PE"].apply(_pe_label)

    # Quick cue
    def _cue(row):
        pe = row.get("PE", np.nan)
        c  = row.get("PE_corr", np.nan)
        if not np.isfinite(pe): return "insufficient splits"
        if pe >= 75: return "trip-tolerant; adaptable to pace shifts"
        if pe >= 55: return "generally adaptable; minor shape caveats"
        # For low PE, check whether the correlation is positive or negative
        if np.isfinite(c) and c < 0:
            return "one-rhythm closer; dislikes mid-race slow-down"
        return "needs own way / controlled fractions"
    view["PE_cue"] = view.apply(_cue, axis=1)

    # Sort by (GCI desc, then PE desc) — see class × adaptability
    view = view.sort_values(["GCI","PE"], ascending=[False, False]).reset_index(drop=True)
    st.dataframe(view, use_container_width=True)
# ======================= /Pace Elasticity (PE) — RAM-light table =======================
