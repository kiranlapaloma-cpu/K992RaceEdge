...
def _std01(x):
    ...
    x = x[np.isfinite(x)]
    if x.size == 0: return np.nan
    mu, sd = np.nanmedian(x), np.nanstd(x)
    if sd <= 1e-9: return (x - mu) * 0.0
    return (x - mu) / (sd + 1e-9)

def _ensure_devs(df, early_col=None, late_col=None):
    """
    Returns early_dev, late_dev series.
    Accepts either *_dev columns directly OR EARLY_idx/LATE_idx (100 = baseline).
    """
    cols = {c.lower(): c for c in df.columns}
    # Priority 1: already-computed *_dev
    e_dev = None
    l_dev = None
    if early_col and early_col in df.columns: e_dev = _as_num(df[early_col])
    if late_col  and late_col  in df.columns: l_dev = _as_num(df[late_col])
    if e_dev is None:
        for cand in ["early_dev", "earlydev", "early_deviation"]:
            if cand in cols: e_dev = _as_num(df[cols[cand]]); break
    if l_dev is None:
        for cand in ["late_dev", "latedev", "late_deviation"]:
            if cand in cols: l_dev = _as_num(df[cols[cand]]); break
    # Priority 2: derive from indexes (100 ~ neutral)
    if e_dev is None:
        for cand in ["early_idx","earlY_idx","earlyindex","early_index"]:
            if cand in cols: 
                e_dev = _as_num(df[cols[cand]]) - 100.0
                break
    if l_dev is None:
        for cand in ["late_idx","lateindex","late_index"]:
            if cand in cols: 
                l_dev = _as_num(df[cols[cand]]) - 100.0
                break
    # Last resort: zeros (no signal)
    if e_dev is None: e_dev = pd.Series(np.zeros(len(df)), index=df.index, dtype=float)
    if l_dev is None: l_dev = pd.Series(np.zeros(len(df)), index=df.index, dtype=float)
    return e_dev, l_dev

def detect_shape_bias(df,
                      early_col=None,
                      late_col=None,
                      finish_col="Finish_Pos",
                      time_col=None):
    """
    Returns: dict(summary) and per-horse cue Series.
    Summary keys:
      - label: 'Front-biased' | 'Closer-biased' | 'Fair'
      - severity: 0..1 (how strong the bias is)
      - components: dict with the sub-scores for debugging/insight
    """
    e_dev, l_dev = _ensure_devs(df, early_col, late_col)
    # Rank: lower is better. If no finish column, fallback to time; else best-effort.
    if finish_col in df.columns:
        rank = _as_num(df[finish_col])
    elif time_col and time_col in df.columns:
        # Smaller time => better rank
        rank = _as_num(df[time_col]).rank(method="average", ascending=True)
    else:
        # If no rank/time, treat top half of rows as "better" just to keep it running.
        rank = pd.Series(np.arange(len(df))+1, index=df.index, dtype=float)

    # Normalize devs for robust comparability
    e_z = pd.Series(_std01(e_dev), index=df.index)
    l_z = pd.Series(_std01(l_dev), index=df.index)

    # 1) Correlation signals (Spearman resistant to outliers)
    def _spearman(a, b):
    ...
    # Positive front_corr => better rank with bigger early_dev (front help)
    front_corr = -_spearman(e_z, rank)  # minus because smaller rank is better
    close_corr =  _spearman(l_z, rank)  # positive => better rank with bigger late_dev

    # 2) Head-to-head split (top half vs bottom half by rank)
    med_rank = np.nanmedian(rank)
    top = rank <= med_rank
    e_gap = np.nanmean(e_z[top]) - np.nanmean(e_z[~top])  # positive => early helped winners
    l_gap = np.nanmean(l_z[top]) - np.nanmean(l_z[~top])  # positive => late helped winners

    # 3) Global skew: field-wide leaning to early vs late usage
    skew = np.nanmedian(e_z) - np.nanmedian(l_z)

    # Combine with sensible weights (sum to 1)
    # front_score > 0 ⇒ speed bias; < 0 ⇒ closer bias
    front_score = 0.45*front_corr + 0.35*e_gap + 0.20*skew
    closer_score = 0.45*close_corr + 0.35*l_gap - 0.20*skew
    net = front_score - closer_score  # positive => front bias

    # Severity from |net|, soft-saturated to 0..1
    severity = float(np.tanh(abs(net) * 1.1))  # gentle saturation

    if   net >=  0.20: label = "Front-biased"
    elif net <= -0.20: label = "Closer-biased"
    else:              label = "Fair"

    components = {
        "front_corr": round(front_corr, 3),
        "close_corr": round(close_corr, 3),
        "e_gap": round(e_gap, 3),
        "l_gap": round(l_gap, 3),
        "skew": round(skew, 3),
        "net": round(net, 3)
    ...
    # Per-horse cue: aligns or fights the bias?
    def _cue_row(e, l):
        if label == "Front-biased":
            if e > 0.25 and l < 0.25:  return "with shape"
            if e < -0.25 and l > -0.25:return "against shape"
            return "neutral"
        if label == "Closer-biased":
            if l > 0.25 and e < 0.25:  return "with shape"
            if l < -0.25 and e > -0.25:return "against shape"
            return "neutral"
        # Fair
        return "neutral"

    cues = pd.Series([_cue_row(ez, lz) for ez, lz in zip(e_z, l_z)], index=df.index, name="RSI_Cue")

    summary = {"label": label, "severity": severity, "components": components}
    return summary, cues

# ----------------- Convenience wrapper to decorate a DataFrame --------------
def apply_shape_bias_module(df,
                            early_col=None, late_col=None,
                            finish_col="Finish_Pos", time_col=None,
                            cue_col_name="RSI_Cue",
                            attach_banner=True):
    summary, cues = detect_shape_bias(df, early_col, late_col, finish_col, time_col)
    out = df.copy()
    out[cue_col_name] = cues
    if attach_banner:
        try:
            import streamlit as st
            ...
            bar = "█" * int(10*sev) + "░" * (10-int(10*sev))
            st.subheader("Race Shape (one-run inference)")
            st.info(f"**{label}**  •  severity {sev:.2f}  •  {bar}\n\n"
                    f"`front_corr` {comp['front_corr']} | `close_corr` {comp['close_corr']} | "
                    f"`e_gap` {comp['e_gap']} | `l_gap` {comp['l_gap']} | `skew` {comp['skew']}")
        except Exception:
            pass
    return out
# =================== /Shape Bias Detector ====================================
