...
def _dist_blend(dm: float):
    """Weights for speed vs sustain by distance (sum=1). ≤1200: 0.65/0.35 … ≥2000: 0.35/0.65."""
    if not np.isfinite(dm):
        return 0.50, 0.50
    if dm <= 1200:  return 0.65, 0.35
    if dm >= 2000:  return 0.35, 0.65
    t = (dm - 1200.0) / (2000.0 - 1200.0)
    w_speed = 0.65*(1 - t) + 0.35*t
    return float(w_speed), float(1.0 - w_speed)

# --- Column plumbing (accepts multiple naming variants) ---
vmax = None
for c in ["Vmax (km/h)", "Vmax_kmh", "Vmax"]:
    if c in VP.columns: vmax = _as_num(VP[c]); break

sus_s = None
for c in ["Sustain (s>=95%)", "Sustain_s95", "Sustain_s"]:
    if c in VP.columns: sus_s = _as_num(VP[c]); break

sus_m = None
for c in ["Sustain (m>=95%)", "Sustain_m95", "Sustain_m"]:
    if c in VP.columns: sus_m = _as_num(VP[c]); break

rt_s  = _as_num(VP.get("RaceTime_s"))
dm    = None
# prefer an attached distance; otherwise try app/global variables
for k in ["Distance_m", "race_distance_m", "RaceDistance_m"]:
    if k in VP.columns: dm = float(_as_num(VP[k]).median()); break
if dm is None:
    # common places you store this:
    dm = float(metrics.attrs.get("DIST_M", np.nan) if hasattr(metrics, "attrs") else np.nan)
    try:
        dm = float(race_distance_input) if (not np.isfinite(dm)) else dm
    except Exception:
        pass

# --- Factors ---
# Speed factor: compare Vmax to field P75 (robust, one-run, distance-agnostic)
if vmax is not None:
    vref = np.nanpercentile(vmax, 75) if np.isfinite(vmax).any() else np.nan
    sf = vmax / (vref if (vref and vref > 0) else np.nan)
    sf = _clip(sf, 0.80, 1.20)                           # winsorize against spikes
    SF01 = (sf - 0.80) / 0.40                            # 0–1 scale
else:
    SF01 = pd.Series(np.nan, index=VP.index)

# Sustain factor: time share at ≥95% (optionally blend metres for shape-robustness)
if sus_s is not None and rt_s is not None:
    uf_time = sus_s / _clip(rt_s, 1e-6, 1e9)
else:
    uf_time = pd.Series(np.nan, index=VP.index)

if sus_m is not None and np.isfinite(dm):
    uf_metres = sus_m / max(dm, 1.0)
else:
    uf_metres = pd.Series(np.nan, index=VP.index)

# Blend (lean to time; metres stabilises bend/track layout artefacts)
UF01 = np.nan_to_num(0.7*np.asarray(uf_time) + 0.3*np.asarray(uf_metres), nan=np.nan)
UF01 = pd.Series(UF01, index=VP.index)
UF01 = _clip(UF01, 0.0, 1.0)

# Tiny onset bonus (earlier sustain -> small bump; never a penalty)
onset = _as_num(VP.get("Onset_from_home_m"))
if onset is not None and np.isfinite(onset).any():
    on01 = 1.0 - _percentile01(onset)
    OnBonus = 0.05 * _clip(on01, 0.0, 1.0)
else:
    OnBonus = pd.Series(0.0, index=VP.index)

# Distance-aware weights (speed vs sustain)
wS, wU = _dist_blend(dm)

# Geometric blend keeps “×” spirit and prevents one leg from dominating
Grade_raw = np.power(SF01, wS) * np.power(UF01, wU)

# Final 0–10 grade, plus expose sub-factors for debug
VP["Grade10"]  = _clip(10.0 * Grade_raw + OnBonus, 0.0, 10.0).round(2)
VP["SF01"]     = SF01.round(3)
VP["UF01"]     = UF01.round(3)
VP["OnBonus"]  = OnBonus.round(3)

# (Optional) Show right next to your V-Profile table
# order_cols = ["Horse","VProfile","TSI","SSI","Vmax (km/h)","Sustain (s>=95%)","Sustain (m>=95%)","Grade10","SF01","UF01","OnBonus"]
